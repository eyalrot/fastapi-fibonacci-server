...
def test_fibonacci_endpoint():
    ...
    for algo in algorithms:
        ...
        print(f"✓ Fibonacci(20) with {algo}: {data['result']} (time: {data['computation_time_ms']:.3f}ms)")
...
def test_benchmark():
    ...
    for impl, result in sorted(data["results"].items()):
        if "time_ms" in result and result["time_ms"] is not None:
            print(f"{impl:20} | {result['time_ms']:10.3f}ms | result: {result['result']}")
        else:
    ...
    assert all(r == results[0] for r in results), "All implementations should return the same result"
